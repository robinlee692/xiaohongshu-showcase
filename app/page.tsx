        ...
        <div className="max-w-[1600px] mx-auto px-4 py-4">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="w-4 h-4 text-xhs-red rounded focus:ring-xhs-red"
            />
            ...
            {posts.map((post) => (
              <div key={post.slug} className="relative">
                <input
                  ...
                  className="absolute top-2 left-2 w-4 h-4 text-xhs-red rounded focus:ring-xhs-red z-10"
                />
                  ...
                  <div className="xhs-card h-full cursor-pointer pt-8">
                    <div className="p-4">
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <select
                          value={post.status || '待发布'}
                          onChange={(e) => {
                            e.preventDefault()
                            e.stopPropagation()
                            handleStatusChange(post.slug, e.target.value)
                          }}
                          onClick={(e) => e.stopPropagation()}
                          className={`px-2 py-0.5 text-xs rounded-full font-medium border-0 cursor-pointer ${
                            post.status === '已发布' 
                      ...
                      {post.tags && post.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 mb-3">
                          {post.tags.slice(0, 3).map((tag, index) => (
                            <span
                              key={index}
                              className="px-1.5 py-0.5 bg-xhs-pink bg-opacity-20 text-xhs-red text-[10px] rounded-full"
                            >
                              #{tag}
                            </span>
                          ))}
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider px-4 py-3">标题</th>
                  <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider px-4 py-3">日期</th>
                  <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider px-4 py-3">标签</th>
                  <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider px-4 py-3">字数</th>
                  <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider px-4 py-3">配图</th>
                </tr>
              </thead>
                  ...
                  <tr key={post.slug} className="hover:bg-gray-50 cursor-pointer" onClick={() => window.location.href = `/posts/${post.slug}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {post.tags?.slice(0, 3).map((tag, index) => (
                          <span key={index} className="px-2 py-0.5 bg-xhs-pink bg-opacity-20 text-xhs-red text-xs rounded-full">
                            #{tag}
                          </span>
                        ))}
                    ...
                    <td className="px-4 py-3">
                      <span className="text-sm text-gray-500">{post.content.length}字</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-sm text-gray-500">{post.imageCount || 0}图</span>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600">
                确定要删除选中的 <span className="font-bold text-xhs-red">{selectedPosts.size}</span> 篇内容吗？
              </p>
